...
'title' => 'User Admin',
'contentTitle' => 'Tambah User Admin'
])
@section('content')
            ...
            <div class="card-body">
                <form action="/admin/user" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="username" class="form-control @error('username') is-invalid @enderror" id="username" value="{{ old('username') }}" onkeyup="this.value = this.value.replace(/[^a-z0-9-_.]/g, '')" required>
                        @error('username')
                        <div class="invalid-feedback">
                            {{ $message }}
                        </div>
